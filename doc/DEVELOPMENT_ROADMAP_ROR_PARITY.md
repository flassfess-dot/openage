...
**Статус E3 save/load vertical, 2026-09-15:** меню получило работающий quick-save/quick-load, а версия `1` хранит identity/fingerprint матча, fixed tick, итоговый canonical SHA-256, replay с будущими командами, AI и отдельное view/control state. Загрузка сначала воспроизводит матч в отдельном мире и подменяет живой только после совпадения хеша и успешного восстановления всех AI; повреждённый архив не меняет текущую игру. После загрузки запись команд продолжается. Impact-gate проходит для формата, replay, modal UI и main scene; полный E3 gate не запускался до окончания матрицы. Контракт: `doc/ror-modern/GAME_SAVE_CONTRACT.md`. Следующий пакет — E3 state matrix для economy/build/formation/combat/naval с тем же round-trip oracle, затем gap-аудит первого незакрытого владельца ядра.
