...
Статус этапа: `COMPLETE — INTEGRATED`
...
Основные небольшие fixtures: `res://tests/fixtures/e3_save_state_matrix_match.json` и `res://tests/fixtures/e3_controlled_skirmish_match.json`. Они не являются кампаниями и не расширяют runtime-контент.
...
| Непрерывный игровой цикл | Один матч через реальный `main` последовательно выполняет управление, экономику, строительство, производство, развитие, формационный бой и победу без прямого изменения мира тестом | `test_e3_controlled_skirmish.gd` | `PASS` |
...
## Непрерывный controlled skirmish

`test_e3_controlled_skirmish.gd` запускает два одинаковых матча поверх настоящей сцены `main.tscn` и управляет игрой только через player selection, HUD signals, world clicks и публичные commands. Он проверяет последовательность:
...
Harness не выставляет напрямую `task`, HP, ресурсы, очередь или результат матча. Разрешены только fixture-данные до bootstrap, управление выделением и публичные команды. Вторая ветка сохраняется после развития, откатывает несколько тиков загрузкой и приходит к тому же итоговому canonical SHA-256, что непрерывная ветка.

## Граница E3

- Полный автоматический набор: `192 passed / 0 failed`.
- Cache validation: `0 errors / 181 source-owned warnings`; оригинальные ресурсы не конвертировались повторно.
- Windows PCK: `246 998 356` байт, SHA-256 `c9e65cfb3e979e7af4057c889b0dfb9e75dcafc354ab95338232123f40d050f3`.
- Экспортированный EXE автономно запустил `prototype_skirmish` в headless-режиме и завершился с кодом `0` без runtime/script errors.
- E3 закрыт как стабильный `INTEGRATED`, не `PARITY/HARDENED`: точные executable-сверки остаются E8, а массовые и длительные бюджеты — E6.

Активная очередь переходит к E4: построить полную source-derived матрицу всех цивилизаций, общего roster, технологий, бонусов, ограничений, графических и звуковых вариантов; затем закрывать пробелы вертикальными data-driven волнами без сценарных веток.
