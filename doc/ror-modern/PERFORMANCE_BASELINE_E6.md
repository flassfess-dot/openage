...
## 25. E6-021 — воспроизводимый mixed-match baseline

`mixed_match` сохраняет полную популяцию 500 на игрока, но задаёт одинаковый правдоподобный состав нагрузки: 40% собирают ресурсы, 20% идут собранными формациями, 20% ведут длительный ближний бой и 20% остаются резервом. Все активные роли используют обычные authoritative commands и systems; резерв существует в мире и входит в population, spatial/fog/victory и snapshot lifecycle, но не маскируется под отклонённую команду. Боевые участники получили только увеличенный HP fixture, чтобы контакт не исчезал до измерительного окна. Карта, каталог, экономика, формации и canonical hash не имеют benchmark-only runtime ветвей.

Два независимых контрольных запуска `2×100 / 10+10` завершились одинаковым SHA-256 `2d0f1b355cc3e41b52a2985505f7412472707098956db7fd37ed26ba4814f580`. В обоих случаях все 160 предназначенных активных ролей приняли приказ, 40 участников остались резервом, а итоговые task counts сохранились как 80 gather / 40 move / 40 attack / 40 idle.

Основная матрица `400×400 / 520+240`, native path/movement/visibility включены:

| Профиль | Активно / резерв | fixed p50 / p95 / max | unit orders p95 | fog p95 | sample wall | Canonical hash |
|---|---:|---:|---:|---:|---:|---|
| 2×500 | 800 / 200 | 39,746 / 50,421 / 86,085 мс | 35,064 мс | 6,742 мс | 9,85 с | `376a80…9572` |
| 4×500 | 1600 / 400 | 85,202 / 106,585 / 125,797 мс | 72,559 мс | 14,991 мс | 21,09 с | `1b0e1e…29de` |
| 8×500 | 3200 / 800 | 177,885 / 218,330 / 253,209 мс | 149,998 мс | 29,733 мс | 43,41 с | `8ae0ab…9fca` |
...
## 29. E6-026 — достижимость перцепции, точечные ревизии сетки и hot-path аллокации

Аудит кода после E6-025 выявил четыре измеренных и несколько малых владельцев, не закрытых предыдущей кампанией. Пакет закрыт одним прогоном, каждая правка сохраняет игровые правила; эквивалентность доказана побитовым сравнением workload-итогов и счётчиков, а не только тестами.

Изменённые владельцы:

1. **Достижимость цели — один маршрут-запрос вместо запроса на кандидата.** `best_combat_target` вызывал `can_unit_reach_entity` (полный `find_path` с `nearest_walkable` и строковым ключом кэша) для каждого кандидата, прошедшего дальность/видимость; атакующие и `attack_move`-юниты делают это каждый тик. Ранжирование — полный порядок с EntityId-tie-break, поэтому теперь проверяется только победитель; недостижимый победитель запускает один точный fallback-проход со старым фильтром (результат идентичен). Тот же паттерн применён к выбору цели хищников в `wildlife_behavior_system`.
2. **Точечные ревизии навигационной сетки.** Прежде любой жизненный цикл (добавление ресурса, постройка/разрушение/завершение здания, возрастной upgrade, истощение ресурса) вызывал полную пересборку: `configure_terrain_ids` выполнял per-cell Callable по всей карте и безусловно поднимал `revision`. Теперь: лес-тайлы обновляются точечно через `set_terrain_id` при регистрации/снятии дерева; `occupy` идемпотентен и поднимает ревизию только при реальном изменении; `rebuild` строит желаемую карту занятости и диффует её с текущей без deep-copy; все runtime-места перешли на per-entity дельты (`_sync_building_navigation_occupancy`/`_release_building_navigation_occupancy`), полная пересборка осталась только в bootstrap/bulk-финализации.
3. **Журнал изменений сетки и выживание конвертов.** `NavigationGrid` ведёт ограниченный журнал changed-cells с `change_region_since(old_revision)`; конверт общего марша, чей прямоугольник не пересекает ни одного изменившегося региона, только перематывает свою ревизию вместо смерти. Одно срубленное дерево на другом конце карты больше не сбрасывает формации в покомпонентный расчёт. `find_path` очищает route cache при смене ревизии (ключи встраивают ревизию, старые записи всё равно мертвы — теперь память ограничена).
4. **Горячие аллокации.** `update_capturable_objectives` — spatial-запрос + единственный min-scan вместо O(C×N) прохода со словарями и сортировкой, радиус захвата кэширован по kind; payload-словари доменных событий (gather/deposit/decay/attack/hit/damage/projectile) строятся только при включённом `capture_domain_events`; `ai_distress_system.advance` больше не сортирует до 4 096 ключей каждый тик.
5. **Render/frame-путь.** Эффекты сливаются с retained-очередью линейным merge двух отсортированных списков (строгий полный порядок → результат идентичен полной сортировке) вместо `duplicate()` всей очереди и повторной сортировки каждого кадра; каждый drawable несёт кэшированную `screen_position` — проекция выполняется один раз (интерполяция в refresh, статики только при движении камеры: аффинная проекция сохраняет относительный порядок глубины). Поправка E6-026c: на кадрах публикации кэшированные resource/environment статики сохраняли позицию прошлого refresh (пользовательский дефект «деревья скользят с камерой»); rebuild-ветка теперь завершается принудительной статической перепроекцией, закреплено интеграционным тестом `test_panned_static_projection.gd`; сигнатура fog-чанка сравнивается построчным memcmp `PackedByteArray` вместо 1024-итерационного хеша; презентационный `terrain_id_at_cell` использует O(1) индекс леса мира вместо линейного сканирования всех ресурсов на клетку (стоило мультимиллисекундных спайков при панорамировании).

Контрольные измерения (`gather_economy 2×500 / 400×400 / 520+240`, native, одна машина, одна рабочая копия; baseline — git stash HEAD):

| Метрика | HEAD | E6-026 | Изменение |
|---|---:|---:|---:|
| gather workload p95 fixed tick | 50,53 мс | 49,97–54,84 мс (два прогона) | в пределах шума машины (±7–9% run-to-run) |
| setup wall | 17,77 с | 13,99–14,63 с | `−21%` (нет полной пересборки на каждый add) |
| capturable objectives p95 | 1,11 мс | 0,01 мс | `~100×` |
| workload-итоги (gather/deposit/food/carrying) | 11 671 / 988 / 5060+5000 / 693 | идентичны побитово | эквивалентность |
| probe counters (path/native/direct) | — | идентичны | эквивалентность |

`combat_contact 8×500 / 400×400 / 2+3`:

| Метрика | HEAD | E6-026 | Изменение |
|---|---:|---:|---:|
| fixed tick p95 | 534,73 мс | 420,72 мс | `−21%` |
| unit_orders p95 | 424,69 мс | 323,30 мс | `−24%` |
| attack task p95 | 175,81 мс | 136,44 мс | `−22%` |
| animation p95 | 193,18 мс | 142,81 мс | `−26%` |
| счётчики (path queries/native updates/direct hits) | — | идентичны | эквивалентность |

Canonical hash изменился один раз и принят намеренно (прецедент E6-009/E6-012): серийное поле `path_grid_revision` отражает число bump'ов ревизии, а не игровое состояние, и его значения сдвинулись из-за точечных ревизий. Доказательство эквивалентности: побитово равные workload-итоги и probe-счётчики в обоих профилях, самодетерминизм (два прогона — один hash), полный `test_deterministic_replay` и save/replay impact-тесты проходят.

Полный suite: `210 passed / 4 failed` до правки теста, из них два отказа требуют отсутствующего в среде `node` (`test_cache_contract`, `test_slp_semantics` — падают идентично на HEAD), `test_entity_lookup_index` обновлён под флаговый контракт purge E6-019 (падал и на HEAD), `test_order_pipeline` воспроизводит до-существующий дефект: воркер, заспавненный вплотную к ресурсу, молча завершает gather-приказ без одного цикла (владелец — I8 approach-slot контракт; диагностика: history `AcquireTarget → PlanPath → MoveIntoRange → RepeatOrComplete`, task `idle`, 0 циклов за 60 тиков).

Не измерено в этом пакете (нет GPU/дисплея в среде прогона): frame-выигрыши п.5 требуют повторного `benchmark_e6_visible` / live packaged probe. Оставшиеся владельцы: HUD rebuild по сигнатуре (пересекается с E3R-001), `_attacker_metadata` и roster-refresh churn по entity-count, инкрементальные счётчики живых юнитов для victory, O(G²)+G×U broad-phase изоляции формаций, пары рабочий×ресурс в AI-экономисте, полная навигационная проекция карты на каждое решение AI.
