...
> **Статус после core-first rebaseline 2026-09-14: `FROZEN_BACKLOG`.** Матрица и уже опубликованные кампании остаются regression fixtures. Новые кампании не интегрируются до прохождения visual/HUD core, полного gameplay core, всех цивилизаций, skirmish/random maps/AI и performance stage. Исторический ranking ниже сохранён как готовая точка возобновления, а не как текущая очередь.
...
Завершённая `First Punic War` больше не участвует в списке кандидатов. Первым сохранённым кандидатом на этап E7 выбрана `Reign of the Hittites`, score `[3, 4, 5, 5]`. Это не текущая задача. Её будущий общий системный пакет:
...
После разморозки E7 миссии не импортируются как пять несвязанных прототипов. Сначала закрываются общие bootstrap/condition/asset owners, затем создаются все пять fixed-source matches и только после воспроизводимых исходов кампания публикуется целиком. До E7 portfolio можно читать и использовать для regression/audit, но его gaps не определяют порядок разработки ядра.
