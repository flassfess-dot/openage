...
/// Failed to decode a string as WINDOWS-1251.
///
...
#[error("could not decode string as WINDOWS-1251")]
pub struct DecodeStringError;

/// Failed to encode a string as WINDOWS-1251.
///
...
#[derive(Debug, Clone, Copy, thiserror::Error)]
#[error("could not encode string as WINDOWS-1251")]
...
pub fn write_str<W: Write>(output: &mut W, string: &str) -> Result<(), WriteStringError> {
    let (bytes, _enc, failed) = WINDOWS_1251.encode(string);
    if failed {
...
/// Decode a string using the WINDOWS-1251 code page.
fn decode_str(bytes: &[u8]) -> Result<String, DecodeStringError> {
    if bytes.is_empty() {
        return Ok("".to_string());
    }

    let (decoded, _enc, failed) = WINDOWS_1251.decode(bytes);
    if failed {
