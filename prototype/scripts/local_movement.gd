...
static func calculate_runtime_unit_into(unit: Dictionary, target: Vector2, neighbors: Array, navigation_grid, delta: float, open_envelope: Variant = null) -> String:
	...
	var own_radius := float(unit["footprint_radius"])
	var own_clearance := float(unit["minimum_clearance"])
	var own_priority := int(unit["push_priority"])
	var avoidance := Vector2.ZERO
	for other in neighbors:
		if float(other["hp"]) <= 0.0:
			continue
		var other_position: Vector2 = other["previous_pos"]
		var gap := position - other_position
		var distance := gap.length()
		var safe_distance := own_radius + float(other["footprint_radius"]) + maxf(own_clearance, float(other["minimum_clearance"]))
		if distance <= 0.0001:
			var sign_value := -1.0 if unit_id < int(other["id"]) else 1.0
			gap = Vector2(0.0, sign_value)
			distance = 0.0001
		if distance < safe_distance * 1.6:
			var strength := clampf((safe_distance * 1.6 - distance) / (safe_distance * 1.6), 0.0, 1.0)
			var gap_normalized := gap.normalized()
			var other_priority := int(other["push_priority"])
			var displacement_share := 0.5 if own_priority == other_priority else (0.75 if own_priority < other_priority else 0.25)
			avoidance += gap_normalized * speed * strength * displacement_share
			var to_other := -gap_normalized
			if has_desired and desired_normalized.dot(to_other) > 0.55:
