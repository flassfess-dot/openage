...
func _initialize() -> void:
	...
	assert_equal(after["queue"].size(), 1, "next snapshot updates the HUD queue")
	...
	assert_equal(barracks["production_queue"].size(), 0, "authoritative queue removes cancelled item")
	assert_equal(world.get_resource_amount(1, 0), 180, "cancellation refunds authoritative cost")
	var cancelled: Dictionary = view_model.build(SimulationSnapshot.presentation(world, controller.tick_index, 1), [80], "RECTANGLE", "ru")
	assert_equal(cancelled["queue"].size(), 0, "following snapshot removes cancelled queue item")
