...
func verify_faith_and_priest_technologies(world, priest: Dictionary) -> void:
	...
	assert_near(float(conversion.get("faith", 0.0)), 80.0, 0.0001, "Fanaticism raises recharge to three points per second")
	assert_near(float(conversion.get("recharge_rate", 0.0)), 3.0, 0.0001, "Fanaticism rate is visible in component state")
