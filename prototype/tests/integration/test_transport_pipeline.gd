...
func verify_conversion_and_destruction_policy(catalog) -> void:
	...
	assert_equal(int(passenger.get("team", -1)), 1, "conversion preserves the passenger's original owner")
	assert_true(conversion_world.find_unit(int(passenger["id"])) != null, "incompatible passenger disembarks on the available shore")
	assert_true(not conversion_world.transport_system.embarked_units.has(int(passenger["id"])), "ejected passenger leaves the captured transport cargo")
	assert_equal(transport["components"]["cargo"]["passenger_ids"], [], "captured transport manifest clears after safe ejection")
	...
	assert_equal(String(enemy_controller.get_command_result(enemy_unload.sequence_id).get("reason", "")), "transport_empty", "new owner cannot unload a passenger already ejected on conversion")
