...
func _initialize() -> void:
	...
	assert_true(registry.are_allied(1, 3), "directed alliance is visible to its owner")
	assert_true(not registry.are_allied(3, 1), "reverse relation remains independent")
	registry.set_relation(3, 1, "neutral")
	assert_equal(registry.relation(3, 1), "neutral", "neutral is a first-class directed relation")
	registry.set_mutual_relation(1, 3, "ally")
	assert_true(registry.are_allied(3, 1), "explicit mutual relation updates the reverse direction")
	...
	assert_equal(registry.relations_for(3), {1: "ally", 2: "enemy", 3: "ally"}, "public relation row is complete and stable")
