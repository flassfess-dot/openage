...
func _initialize() -> void:
	...
	assert_equal(entries.size(), 10, "launcher exposes the prototype and nine proven campaign verticals")
	...
	assert_equal(entries.map(func(entry): return String(entry.get("id", ""))), ["prototype_skirmish", "campaign_birth_of_rome", "campaign_pyrrhus_of_epirus", "campaign_syracuse", "campaign_metaurus", "campaign_zama", "campaign_mithridates", "campaign_struggle_for_sicily", "campaign_battle_of_mylae", "campaign_battle_of_tunes"], "launcher preserves campaign and mission order around the prototype")
