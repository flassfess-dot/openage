...
func _initialize() -> void:
	...
	assert_equal(entries.size(), 7, "launcher exposes the prototype and all six proven campaign verticals")
	...
	assert_equal(entries.map(func(entry): return String(entry.get("id", ""))), ["prototype_skirmish", "campaign_birth_of_rome", "campaign_pyrrhus_of_epirus", "campaign_syracuse", "campaign_metaurus", "campaign_zama", "campaign_mithridates"], "launcher preserves the complete campaign order around the prototype")
